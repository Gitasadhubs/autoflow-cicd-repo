...
import { ClipboardIcon, ClipboardCheckIcon, CodeBracketIcon, CheckCircleIcon, LockClosedIcon, EyeIcon, EyeSlashIcon, LogoIcon } from './icons';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
      <div className="bg-brand-surface rounded-xl shadow-2xl w-full max-w-3xl transform transition-all animate-scale-up border border-gray-700">
        <div className="p-6 border-b border-gray-700">
          <h2 className="text-2xl font-bold text-gray-100">Configure Pipeline</h2>
          <p className="text-gray-400 mt-1">For repository: <span className="font-semibold text-brand-primary">{repo.full_name}</span></p>
        </div>
        
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          <fieldset className="p-4 border border-gray-700 rounded-lg">
            <legend className="text-sm font-medium text-gray-300 px-2">Build & Deploy Settings</legend>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                <div>
                <label htmlFor="tech-stack" className="block text-sm font-medium text-gray-300 mb-1">Tech Stack</label>
                <select 
                    id="tech-stack"
                    value={techStack}
                ...
                <div>
                <label htmlFor="deployment-target" className="block text-sm font-medium text-gray-300 mb-1">Deployment Target</label>
                <select 
                ...
                <div>
                <label htmlFor="deployment-environment" className="block text-sm font-medium text-gray-300 mb-1">Environment</label>
                <select 
          ...
          {requiredVariables.length > 0 && (
            <fieldset className="p-4 border border-gray-700 rounded-lg">
                <legend className="text-sm font-medium text-gray-300 px-2">Configure Workflow Variables</legend>
                <p className="text-xs text-gray-400 pb-3">These non-sensitive variables will be set in your repository settings.</p>
                <div className="space-y-3">
                    {requiredVariables.map(variable => (
                        <div key={variable.name}>
                            <label htmlFor={variable.name} className="block text-sm font-medium text-gray-300">{variable.name}</label>
                            ...
                            <input
                                ...
                                className="w-full bg-gray-900 border border-gray-600 text-gray-200 text-sm rounded-lg focus:ring-brand-primary focus:border-brand-primary block p-2.5 placeholder-gray-500"
                            />
                        </div>
                    ))}
                </div>
          ...
          {requiredSecrets.length > 0 && (
            <fieldset className="p-4 border border-amber-600 rounded-lg bg-amber-500/10">
              <legend className="text-sm font-medium text-amber-300 px-2 flex items-center">
                <LockClosedIcon className="h-4 w-4 mr-1.5" />
                Configure Workflow Secrets
              </legend>
              <p className="text-xs text-amber-400 pb-3">These sensitive values will be stored as encrypted secrets in your repository.</p>
              <div className="space-y-3">
                {requiredSecrets.map(secret => (
                  <div key={secret.name}>
                    <label htmlFor={secret.name} className="block text-sm font-medium text-gray-300">{secret.name}</label>
                    <p className="text-xs text-gray-400 mb-1">{secret.description}</p>
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-gray-900 border border-gray-600 text-gray-200 text-sm rounded-lg focus:ring-brand-primary focus:border-brand-primary block p-2.5 pr-10 placeholder-gray-500"
                      />
                      <button
                        type="button"
          ...
          {generatedYaml && (
            <div className="space-y-2">
                <h3 className="text-sm font-medium text-gray-300">Generated Workflow</h3>
                <div className="relative bg-gray-900 rounded-lg p-4 max-h-60 overflow-auto border border-gray-700">
                    <button
                    ...
                    <pre><code className="text-sm text-gray-200 font-mono">{generatedYaml}</code></pre>
                </div>
            </div>
          )}
        ...
        <div className="px-6 py-4 bg-gray-900/50 flex justify-end space-x-3 rounded-b-xl border-t border-gray-700">
          <button onClick={onClose} className="py-2 px-4 bg-gray-600 text-gray-200 rounded-lg hover:bg-gray-500 transition">Cancel</button>
          <button 
            onClick={handleConfirmSetup}
            disabled={!generatedYaml || isCommitting}
            className="py-2 px-4 bg-status-success text-white rounded-lg hover:bg-green-700 transition disabled:bg-gray-500 disabled:cursor-not-allowed min-w-[150px] flex items-center justify-center font-semibold"
          >
            {isCommitting ? (
              commitStatus === 'Success!' ? <CheckCircleIcon className="h-5 w-5 text-white" /> : <><LogoIcon className="h-5 w-5 animate-rocket-float mr-2" /> <span>{commitStatus || 'Launching...'}</span></>
            ) : "Confirm Setup" }
          </button>
        </div>
      </div>
